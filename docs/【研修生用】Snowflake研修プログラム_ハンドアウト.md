...
### 知っておくべきITリテラシー
#### 秘密鍵の取り扱い
本研修では触れませんが、本研修の構成では秘密鍵・公開鍵による認証が行われています。
秘密鍵を紛失すると、以下のような致命的な問題が発生します。

- Snowflake環境の流出

- TODO

今後、秘密鍵・公開鍵の取り扱いを行う場面も想定されますので、各自ご勉強ください。
...
構成図の作成にはdraw.ioというツールを使用します。手動で一から作成することもできますが、ここではAI（LLM）に構成図のコード（XML）を出力させる方式で課題を行っていただきます。
...
### ＜手順概要＞
- 事前に作成したアカウントでHCP Terraformにログインし、課題①・②を実施します。
