...
3.  [構成図の作成（1.5h）](#chapter-3)

4.  [AWS環境構築（2h）](#chapter-4)

5.  [GitHub環境構築（2h）](#chapter-5)

6.  [Terraformによるsnowflakeオブジェクトの定義（4h）](#chapter-6)

7.  [dbtによるデータ加工（4h）](#chapter-7)

8.  [Streamlitアプリケーションの実装（1h）](#chapter-8)

9. [レポート作成（1h）](#chapter-9)
...
知らない概念やわからない言葉が出てきたら、[Snowflake研修リファレンス](【参考資料】Snowflake研修プログラム_リファレンス.md)を参照することをおすすめします。
...
構成図の作成にはdraw.ioというツールを使用します。手動で一から作成することもできますが、ここではAI（LLM）に構成図のコード（XML）を出力させる方式で手順を記載しています。


### ＜手順概要＞

- [こちら](drawIO_guideline_v2.1.json)に、構成図ガイドライン（.json）があります。これをChatGPTやClaude等、任意のAIチャットに添付します。

- ガイドライン(json)に沿って、課題を満たす内容の構成図XMLをAIに出力させる。
...
- 以下のリンクはS3のバケットを示しています。こちらにある**qz-trainee-pipeline.yml**をダウンロードします。こちらがCloudFormationのテンプレートとなりますが、中身は不完全な状態になっています。課題に沿って、このファイルをローカルで編集します。
...
- [snowflake_training_for_traineeリポジトリ](https://github.com/ti2140/snowflake_training_for_trainee)を開き、リポジトリをローカルにクローンします。
- 課題①に沿って、必要な環境変数を登録します。
- 課題②を実施し、SharePointの成果物提出フォルダに提出してください。
- 課題③～④を実施してください。
...
## 6. TerraformによるSnowflakeオブジェクトの定義（6h）
...
### ＜手順概要＞
- 事前に作成したアカウントでHCP Terraformにログインし、課題①・②を実施します。課題②で作成したテキストファイルは、成果物フォルダに提出してください。

- その後、課題③～⑤に沿って、対応するソースコードをローカルにて編集します。

- 課題⑥・⑦を実施します。CI/CDパイプラインの**Terraformステップが**エラーなく動き、今回定義したオブジェクトが生成されていれば完了です。

### ＜課題＞
① 組織「snowflake-training」内に、任意の名称でworkspaceを作成してください。種類はAPI-Driven Workflowを選択してください。  
...
また、Categoryとして"Terraform variable"または"Environment variable"を選択できました。Terraform変数と環境変数の違いを説明したテキストを作成してください。
...
- 課題①・②に沿ってdbtソースコードをローカルにて編集し、課題③を実施します。
...
- レポートを作成し、Sharepointの成果物提出フォルダに提出します。

### ＜課題＞

- 新たなStreamlitアプリを自由に作成してみてください。
以下の要件を参考にしてみるのもアリです。
また、これまでの研修内容を参考に、新たな環境を構築してみても問題ないです。

**【参考要件】**
- メモ帳にTODOを記載
- LambdaでJSONL形式に加工する
- 加工したデータを取得した、Streamlit TODOアプリを作成する
